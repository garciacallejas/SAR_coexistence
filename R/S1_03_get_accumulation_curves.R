...
str.files <- files[grep("S1_posterior_draws_structural_metrics",files)]
...
if(!is.null(hom.curves)){
  write.csv2(hom.curves,
             file = paste(getwd(),"results/posterior_draws/S1_posterior_draws_accumulation_curves_heterogeneous_time.csv",sep=""),
             row.names = FALSE)
}

if(!is.null(het.curves)){
  write.csv2(het.curves,
             file = paste(getwd(),"results/posterior_draws/S1_posterior_draws_accumulation_curves_heterogeneous_both.csv",sep=""),
             row.names = FALSE)
}
# clear cores -------------------------------------------------------------
