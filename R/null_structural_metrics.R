#' obtain structural stability metrics for null alpha matrices
#' that do not maintain, necessarily, the interaction identities
#' of the original matrices
#' 
#' @param lambdas data frame, year, plot, species, lambda 
#' @param alphas data frame from null_structural_metrics, 
#' year, plot, focal, neighbour, magnitude
#' @param sp.rates data frame, species.code, germination.rate, seed.survival 
#'
#' @return data frame, feasibility, feasibility.domain, com.pair.differential
#' @export
#'
#' @examples
null_structural_metrics <- function(lambdas, alphas, sp.rates){
  ...
  # this bit is unique to the null version
  # and requires to input the homogeneous parameters
  # as it assumes that there is ONLY ONE lambda per sp
  # across all sites
  splambda <- unique(lambdas[,c("sp","lambda.log")])
  splambda <- splambda[complete.cases(splambda),]
  ...
  for(i.year in 1:length(years)){
    ...
    # plots <- c(unique(alpha.year$plot))
    ...
    for(i.plot in 1:n.plots){
      ...
      # present species
      # this is the bit different to the original
      # here, adapt presence to alpha matrix, 
      # instead of the other way around
      
      # lambda.plot <- subset(lambda.plot, !is.na(lambda))
      my.sp <- sort(unique(intersect(alpha.plot$focal,alpha.plot$neighbour)))
      alpha.plot.clean <- subset(alpha.plot, focal %in% my.sp & neighbour %in% my.sp)
      
      # obtain the interaction matrix
      alpha.matrix <- tidyr::spread(alpha.plot.clean,key = neighbour, value = magnitude)
      ...
      if(length(my.sp)>1){
        for(nsp in 2:length(my.sp)){
          ...
          # for each combination, obtain feasibility, feasibility domain
          # and community-pair differential
          for(s in 1:nrow(pw_st)){
            ...
            my.lambdas <- splambda$lambda.log[splambda$sp %in% species]
            ...
            if(valid.combo){
              ...
              pw_st$feasibility[s] <- ifelse(class(feas)=="try-error",NA_real_,feas)
          ...
          feasres <- dplyr::bind_rows(feasres,pw_st)
...
validrows <- rowSums(feasres[,all.sp])
feasres.clean <- feasres[which(!is.na(validrows)),]
