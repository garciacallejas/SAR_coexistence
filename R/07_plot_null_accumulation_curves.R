...
# this is Fig S1 of the manuscript
...
homaddnull <- nullcoex %>%
  ...
  summarise(coexisting.sp = mean(mean.sp),
            maxm.sp = max(max.sp),
            minm.sp = min(min.sp))
names(homaddnull)[c(6,7)] <- c("max.sp","min.sp")
...
null.coex.plot <- ggplot(homaddob, aes(x = area, y = coexisting.sp))+
  ...
  # richness
  geom_line(data = rich.mean,aes(x = area,y = coexisting.sp),color = palette.values[3],size = 1.2) +
  geom_point(data = rich.mean,aes(x = area,y = coexisting.sp),fill = palette.values[3],shape = 21,size = 3) +
  
  # homogeneous curve
  geom_line(color = palette.values[2],size = 1.2)+
  geom_point(fill = palette.values[2],shape = 21,size = 3)+
...
ggsave(filename = paste("./images/Fig_S1.pdf",sep=""),plot = null.coex.plot,device = cairo_pdf,width = 9,height = 3,dpi = 600)
